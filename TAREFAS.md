...
|------|--------|-----------|
| **🎯 MVP de entrega (reunião 27/08)** | 🟡 Em finalização | **~79%** (só o escopo do MVP; fase posterior não conta) · M2 ✅ · M5 ✅ · M1 quase · M3/M4 do izarley · **apresentação 08/09/2026** |
...
- [x] **Teste end-to-end (CRUD real) — 07/09:** subido um **MariaDB no próprio ambiente**, carregado `schema.sql` + `seed.sql` e rodado o bootstrap (o **Migrator aplicou 001/002/003 sozinho**). Suíte de CRUD real: **15/15 passaram** — imóvel gravando as colunas novas (matrícula/áreas/quartos/custo_seguro), veículo, outro bem, conta, investimento, seguro (vínculo polimórfico), empresa, fornecedor, colaborador, **contrato (M12)** com vínculo + descrição + update, `itensVinculaveis`, `patrimonio_consolidado()`, `alertas_consolidado()` e a **agenda incluindo o contrato**. **M1 = 100%.**

### Bloco M2 — Segurança e controle de arquivos *(Gilson)* ⚠️
- [x] **Controle de acesso a arquivos (05/09) — corrige a lacuna crítica:** antes, tudo em `/uploads/` era **servido direto pelo Apache** (sem login) → qualquer um baixava documento sensível (matrícula, CNH, extrato, contrato) por URL. Agora: bloqueio de acesso direto + **serviço autenticado** `ArquivoController` (rota `/arquivo?doc=<id>` ou `?f=<path>`) que exige login, resolve o **dono pela linha do banco** e autoriza (admin vê tudo; cliente só o próprio — **sem IDOR**). Todas as 13 views migradas para `url_documento()`/`url_arquivo()` (helpers em `functions.php`). Proteção contra path traversal testada.
- [x] **Bloqueio efetivo em produção (07/09):** o deploy FTP **exclui `uploads/**`** (para não apagar os arquivos dos clientes no servidor), então o `uploads/.htaccess` não chegava em produção. Movido o bloqueio para o **`.htaccess` da raiz** (`RewriteRule ^uploads/ - [F,L]`), que é deployado normalmente → `/uploads/` retorna 403 no ar. `uploads/.htaccess` mantido como reforço local.
