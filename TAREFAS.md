...
- [ ] **Revisar campos por categoria de ativo** (imóvel, veículo, jet-ski/embarcação, joia, obra…) — enxugar excesso e definir os campos certos por tipo *(inventário de campos por categoria a montar; decisão de o que cortar depende do César)*
- [ ] **Teste end-to-end** de todas as telas (CRUD real) — **depende de MySQL** (não há DB no ambiente de nuvem; rodar localmente/staging com a migration nova aplicada)
