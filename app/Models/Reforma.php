<?php
/**
 * Model de reforma — tabela `reformas`.
 */
class Reforma
{
    public static function buscar(int $id): ?array
    {
        $s = db()->prepare('SELECT * FROM reformas WHERE id = ?');
        $s->execute([$id]);
        return $s->fetch() ?: null;
    }
    ...
    public static function criar(int $imovelId, array $campos): int
    {
        ...
        db()->prepare('INSERT INTO reformas (' . implode(', ', $cols) . ') VALUES (' . $ph . ')')->execute($vals);
    ...
    public static function atualizar(int $id, array $campos): void
    {
        ...
        db()->prepare("UPDATE reformas SET $set WHERE id = ?")->execute([...array_values($campos), $id]);
    }
}
