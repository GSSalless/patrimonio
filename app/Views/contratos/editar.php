<?php
/**
 * Edição de contrato + documentos.
 * @var array $cli · @var array $contrato · @var array $d · @var string $erro
 * @var string $ok · @var array $docs_list · @var array $itens_vinc
 */
$page_title = 'Editar Contrato — ' . ($contrato['numero'] ?: $contrato['codigo']);
require APP_ROOT . '/includes/header.php';
$vinc_txt = Contrato::descreverVinculo($contrato['vinculo_tipo'], $contrato['vinculo_id'] ? (int)$contrato['vinculo_id'] : null);
?>
<div class="container" style="max-width:960px">
  <div class="card-header" style="margin-bottom:1.25rem">
    <div>
      <div style="font-size:.8rem;color:var(--cor-secundario);font-weight:600"><?= h($contrato['codigo']) ?> · <?= h(contrato_tipo_label($contrato['tipo'])) ?></div>
      <h2 class="card-titulo"><?= h($contrato['objeto'] ?: ($contrato['contraparte_nome'] ?: 'Contrato')) ?></h2>
      <div style="font-size:.85rem;color:var(--cor-secundario)">
        Cliente: <strong><?= h($cli['nome']) ?></strong>
        <?php if ($vinc_txt): ?> · Relacionado a <strong><?= h($vinc_txt) ?></strong><?php endif; ?>
      </div>
    </div>
